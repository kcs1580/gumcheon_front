<template>
  ...
  <div class="example-full main-wrapper">
    ...
    <div class="upload2" v-show="!isOption">
      <div class="table-responsive2 ">
        <v-data-table
          ...
          disable-sort
          hide-default-footer
          class="elevation-1 table table-hover vdatatable2"
          ...
          <template v-slot:[`item.action`]="{ item }">
            <v-btn color="error" @click.prevent="$refs.upload2.remove(item)">
              {{ $t("vueuploadfile.remove") }}</v-btn
            >
            <v-btn
              v-if="item.active"
              @click.prevent="$refs.upload2.update(item, { active: false })"
            >
      ...
      <div class="example-foorer">
        ...
        &nbsp;
        <v-btn
          v-if="!$refs.upload2 || !$refs.upload2.active"
          color="success"
        ...
        ><v-btn
          v-else
          color="error"
          @click.prevent="$refs.upload2.active = false"
...
export default {
  ...
  watch: {
    files2(newValue) {
      this.$store.commit("SET_BOTIMGS", newValue);
    },
    "editFile.show"(newValue, oldValue) {
      // 关闭了 自动删除 error
      if (!newValue && oldValue) {
        this.$refs.upload2.update(this.editFile.id, {
  ...
  methods: {
    ...
    inputFilter(newFile, oldFile, prevent) {
      if (newFile && !oldFile) {
        ...
        if (
          ...
          imageCompressor
            ...
            .then(file => {
              this.$refs.upload2.update(newFile, {
                error: "",
                file,
                size: file.size,
                type: file.type
      ...
      if (
        ...
        img.οnerrοr = e => {
          this.$refs.upload2.update(newFile, { error: "parsing image size" });
        };
      ...
      if (newFile && oldFile) {
        ...
        if (newFile.active && !oldFile.active) {
          ...
          if (
            ...
            this.$refs.upload2.update(newFile, { error: "size" });
          }
        }
    ...
    onEditFileShow(file) {
      ...
      this.$refs.upload2.update(file, { error: "edit" });
    },
    onEditorFile() {
      if (!this.$refs.upload2.features.html5) {
      ...
      this.$refs.upload2.update(this.editFile.id, data);
      this.editFile.error = "";
      this.editFile.show = false;
    },
    // add folder
    onAddFolder() {
      if (!this.$refs.upload2.features.directory) {
        this.alert("Your browser does not support");
        return;
      }
      ...
      input.click();
      input.onchange = e => {
        this.$refs.upload2.addInputFile(input).then(function() {
